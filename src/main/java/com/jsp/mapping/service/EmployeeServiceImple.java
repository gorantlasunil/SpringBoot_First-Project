...
import com.jsp.mapping.dto.BankAccountDTO;
	...
	public EmployeeDTO fetchByEmployeeId(int id) {
		...
		if(optional.isPresent())
		{
			...
			//to transfer data from entity to dTO.........
		      EmployeeDTO finalEmployeeData  = EmployeeDTO
		    		  .builder()
		    		  .ename(employeeFromDB.getEname()).salary(employeeFromDB.getSalary())
		    		  .bankaccount(BankAccountDTO.builder().accountNo(employeeFromDB.getAccount().getAccountNo())
		    				  .IFSCode(employeeFromDB.getAccount().getIFSCode()).build())
		    		  .build();
